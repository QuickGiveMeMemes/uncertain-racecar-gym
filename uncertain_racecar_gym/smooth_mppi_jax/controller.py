...
class JaxSmoothMPPIConfig:
    ...
    lambda_: float = 12.0
    gamma_: float = 0.012
    delta_noise_std: tuple[float, float, float] = (0.10, 0.08, 0.05)
    delta_action_bounds: tuple[float, float, float] = (0.26, 0.18, 0.14)
    action_l2_weight: float = 0.01
    action_diff_weight: float = 0.90
    offtrack_penalty: float = 3500.0
    progress_weight: float = 3200.0
    lateral_weight: float = 24.0
    heading_weight: float = 14.0
    speed_tracking_weight: float = 0.30
    target_speed: float = 20.0
    min_speed: float = 9.0
    speed_profile_quantile: float = 0.72
    speed_profile_scale: float = 0.82
...
class JaxSmoothMPPIController(BenchmarkController):
    def __init__(self, scenario: str, config: JaxSmoothMPPIConfig | None = None):
    ...
    def _ensure_initialized(self, context: ControllerStepContext) -> None:
        ...
        if context.env is None or getattr(context.env, "_state", None) is None:
            raise ValueError("JaxSmoothMPPIController requires a live env with `_state` for warm-start planning.")
    ...
    @staticmethod
    def _smooth_noise(noise: jax.Array) -> jax.Array:
        kernel = jnp.asarray([0.10, 0.20, 0.40, 0.20, 0.10], dtype=jnp.float32)
        padded = jnp.pad(noise, ((0, 0), (2, 2), (0, 0)), mode="edge")

        def smooth_channel(channel_values):
            return jax.lax.conv_general_dilated(
                channel_values[None, :, None],
                kernel[:, None, None],
                window_strides=(1,),
                padding="VALID",
        ...
        def smooth_single(sample):
            channels = [smooth_channel(sample[:, idx]) for idx in range(sample.shape[1])]
            return jnp.stack(channels, axis=-1)

        return jax.vmap(smooth_single)(padded)
    ...
    def _optimize(self, state) -> np.ndarray:
        ...
        for _ in range(self.config.optimization_steps):
            ...
            raw_noise = jax.random.normal(noise_key, shape=(self.config.num_samples, self.config.horizon, 3), dtype=jnp.float32) * sigma
            noise = jnp.clip(self._smooth_noise(raw_noise), -noise_bounds, noise_bounds)
            ...
            smoothness_cost = self.config.action_diff_weight * jnp.sum(jnp.square(action_diff), axis=(1, 2))
