...
class EmpiricalUncertaintyModel:
    ...
    def fit(
        ...
        buckets = {}
        gate_array = np.asarray(gate_rows, dtype=object)
        row_ids_array = np.asarray(row_ids, dtype=int)
    ...
    def sample(
        ...
        if runtime_state.remaining_block > 0 and runtime_state.active_row_id in self.continuation_map:
            ...
            if next_row_id in self._row_lookup:
                ...
                return bucket.residuals[local_index], {"mode": "block", "row_id": int(next_row_id), "gate": gate}
    ...
    def predict_mean(self, feature_vector: np.ndarray, gate_key: tuple[str, str, int]) -> tuple[np.ndarray, dict]:
        if not self.buckets:
            return np.zeros(3, dtype=float), {"mode": "empty"}
        bucket = self._select_bucket(*gate_key)
        if bucket is None or bucket.tree is None:
